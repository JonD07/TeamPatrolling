	...
	while(!valid_solution && K <= boost::numeric_cast<int>(vctrPOINodes.size())) {
		...
		for(int j_g = 0; j_g < input->GetMg(); j_g++) {
			...
			for(int k : depot_order.at(j_g)) {
				...
				for(const std::vector<int> subtour : subtours) {
					...
					/// 7. If there exist drone-tour A such that dist(A) > d^a_max, increase k and repeat steps 2-8
		...
		if(valid_solution) {
			...
			for(int ugv_num = 0; ugv_num < input->GetMg(); ugv_num++) {
				...
				UGVAction initialAction(actionID++, E_UGVActionTypes::e_AtDepot, depots.back().x, depots.back().y, 0);
				...
				for(int drone : drones_to_UGV.at(ugv_num)) {
					// Drone is at the UGV
					DroneAction initialAction(actionID++, E_DroneActionTypes::e_AtUGV, depots.back().x, depots.back().y, 0, ugv_num);
			...
			for(int ugv_num = 0; ugv_num < input->GetMg() && valid_solution; ugv_num++) {
				...
				for(int n : depot_order.at(ugv_num)) {
					...
					// Move to the depot
					{
						...
						UGVAction moveToDepot(actionID++, E_UGVActionTypes::e_MoveToWaypoint, depots.at(n).x, depots.at(n).y, ugv_arrival_time, n);
					...
					for(long unsigned int drone_i = 0; drone_i < depots_tours.at(n).size(); drone_i++) {
						...
						// Launch the drone
						{
							...
							DroneAction launchFromUGV(actionID++, E_DroneActionTypes::e_LaunchFromUGV, ugv_last.fX, ugv_last.fY, completion_time, ugv_num);
							sol_final->PushDroneAction(drone, launchFromUGV);
							UGVAction launchDrone(actionID++, E_UGVActionTypes::e_LaunchDrone, ugv_last.fX, ugv_last.fY, completion_time, drone);
						...
						for(int stop : depots_tours.at(n).at(drone_i)) {
							...
							DroneAction moveToNode(actionID++, E_DroneActionTypes::e_MoveToNode, vctrPOINodes.at(stop).location.x,
						...
						// Move back to UGV
						{
							...
							DroneAction moveToUGV(actionID++, E_DroneActionTypes::e_MoveToUGV, ugv_last.fX, ugv_last.fY, arrival_time, ugv_num);
							// Push action
					...
					while(!arrival_queue.empty()) {
						...
						// Have drone land on UGV
						{
							...
							// Land the drone
							DroneAction landOnUGV(actionID++, E_DroneActionTypes::e_LandOnUGV, ugv_last.fX, ugv_last.fY, landTime, ugv_num);
							sol_final->PushDroneAction(next_arrival.ID, landOnUGV);
							UGVAction landDrone(actionID++, E_UGVActionTypes::e_ReceiveDrone, ugv_last.fX, ugv_last.fY, landTime, next_arrival.ID);
				...
				// Return UGV to base
				{
					...
					// Create action for moving here
					UGVAction moveToDepot(actionID++, E_UGVActionTypes::e_MoveToDepot, depots.back().x, depots.back().y, arrive_time);
					sol_final->PushUGVAction(ugv_num, moveToDepot);
					...
					UGVAction ugvChargeAction(actionID++, E_UGVActionTypes::e_AtDepot, depots.back().x, depots.back().y, kernel_complete_time);
					...
					if(ugvEnergySpent.at(ugv_num) > UGV_TOTAL_BAT) {
						...
						UGVAction ugvEndAction(actionID++, E_UGVActionTypes::e_KernelEnd, depots.back().x, depots.back().y, kernel_complete_time);
						sol_final->PushUGVAction(ugv_num, ugvChargeAction);
						for(int drone : drones_to_UGV.at(ugv_num)) {
							DroneAction endAction(actionID++, E_DroneActionTypes::e_KernelEnd, depots.back().x, depots.back().y, kernel_complete_time);
